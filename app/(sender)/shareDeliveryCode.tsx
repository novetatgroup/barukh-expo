...
const ShareDeliveryCodeRoute = () => {
  ...
  const handleBack = (hasShared: boolean) => {
    router.replace({
      ...
      params: {
        ...trackingParams,
        deliveryCodeShared: String(hasShared),
      },
    });
  };
  ...
  const handleFetchCode = async () => {
    ...
    const response = await senderService.getDeliveryCode(
      trackingParams.shipmentId,
      accessToken
    );

    if (!response.ok || !response.data?.code) {
      throw new Error(response.error || "Unable to load the delivery code.");
    }
  ...
  return (
    <ShareShipmentCodeScreen
      ...
      initiallyShared={trackingParams.deliveryCodeShared === "true"}
      onBack={handleBack}
...
export default ShareDeliveryCodeRoute;
